...
| Native Ableton MIDI tracks | Done | Logic MIDI lands as real MidiTrack/MidiClip elements in the .als, absolute placement |
| Pro Tools import (.ptx) | Done | Clean-room parser: audio regions + trims, track placements, MIDI, stereo lane merge |
| Pro Tools export packages | Done | Spot-to-timestamp WAVs, MIDI files, manifest, import guide (from .als and .logicx) |
| Test suite | Done | 190+ tests across all modules, with real-project coverage gated by local fixtures |
...
Forward lane (logic2ableton) DONE — Logic MIDI notes are extracted from the binary
`ProjectData`, created as **native Ableton MIDI tracks** inside the generated `.als`
(template MidiTrack cloning + MidiClip/KeyTracks/MidiNoteEvent injection mirroring
what Live 12 itself writes), and also exported as importable Standard MIDI files in
`MIDI/`. Notes at or after Logic's bar-1 tick anchor (38400 @ 960 PPQ) keep their
**absolute arrangement positions**; earlier notes fall back to relative placement
with a report warning.

**Cracked binary format (verified against ground truth):** Logic's `ProjectData` is a
chunked binary using little-endian FourCC tags. Arrangement MIDI lives in `EvSq` (stored
`qSvE`) Event Sequence chunks. Each note is a fixed record anchored by the 15-byte
signature `00 00 01 00 00 00 00 00 00 00 89 00 00 00 00` that immediately follows the
`[velocity][pitch]` bytes. Relative to the signature start S:
- velocity = byte at S-2, pitch = byte at S-1
- start position = LE32 at S-9 (ticks, 960 PPQ; project bar-1 origin observed at 38400)
- duration = LE32 at S+15 (ticks, 960 PPQ)
Notes are grouped by their containing `EvSq` chunk (one per region) and timed relative to
the earliest note in the project. Ground truth was generated by writing known `.mid`
files, importing them into Logic Pro, saving as `.logicx`, and decoding the binary against
the known notes — verified across simple, irregular-spacing, and multi-track+chord cases.

**Remaining:** mapping sequences to Logic track names (currently named `MIDI 1`,
`MIDI 2`). Promising lead: each `EvSq` chunk pairs 1:1 with an `MSeq` (`qeSM`) chunk
that carries a u16-length-prefixed name string — observed in real projects but not yet
verified against ground truth, so it stays unshipped per priority #1. Also: some older
Logic saves use a different note-record variant (flag byte `0x69`/`0x88` instead of
`0x89`) that is detected and reported honestly but not yet decoded.
...
## ~~Phase 8: Pro Tools Lanes~~ (DONE)

Shipped in v2.1.0 as four new workflows sharing the existing models and generators:

- **Import:** clean-room `.ptx`/`.pts` parser (`protools_parser.py`) — XOR
  deobfuscation for PT5-9 and PT10+ schemes, recursive block tree, wav list, audio
  regions with three-point trims, region-onto-track placements, MIDI events, sample
  rate, format version. Verified against a real PT2023 96 kHz session and a synthetic
  obfuscated fixture that runs in CI. See `docs/reverse-engineering-pro-tools-sessions.md`.
- **Mapping:** `protools_import.py` merges Pro Tools' per-channel stereo lanes back
  into single tracks, carries source trims through the new `AudioFileRef` offset
  fields, and reports the assumed-tempo caveat honestly.
- **Export:** `protools_transfer.py` produces Pro Tools import packages — per-track
  timestamped WAVs stamped for Spot > Original Time Stamp (session start 00:00:00:00,
  no SMPTE hour offset), per-track MIDI files, manifest, import guide, report.

**Remaining ideas:**
- Extract the session tempo/meter map from `.ptx` (removes the `--tempo` assumption)
- Preserve leading-silence offsets inside MIDI regions (currently anchored to first note)
- Validate legacy `.pts`/`.ptf` (PT 5-9) parsing against real old sessions
- Clip gain and fade shapes
