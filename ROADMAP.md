...
Forward lane (logic2ableton) note transfer is now DONE — Logic MIDI notes are extracted
from the binary `ProjectData` and exported as importable Standard MIDI files in `MIDI/`.
...
**Remaining:** mapping sequences to Logic track names (currently named `MIDI 1`, `MIDI 2`),
and absolute arrangement placement of regions. Notes, velocities, and durations transfer
faithfully; the user places the imported regions on the Ableton timeline.
